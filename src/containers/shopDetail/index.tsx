...
const ShopDetail = memo((props: Props) => {
    ...
    (values: Form) => {
      const onSuccess = () =>
        PopupPrototype.alert(
          _t('success'),
          _t(
            data.current?.id ? 'updateProductSuccess' : 'createProductSuccess',
          ),
          [
            {
              text: _t('ok'),
  ...
  const { setFieldValue, errors, values, isValid } = form;

  useEffect(() => {
    if (!isValid && !errors.imageUri) {
      scrollRef.current?.scrollToPosition(0, 0);
    }
  }, [isValid, errors.imageUri, scrollRef]);
  ...
  const onSelectUnit = useCallback(
    (value: string | number) => setFieldValue('unit', value),
    [setFieldValue],
  );
  ...
  const onBranchItemPress = useCallback(
    (item: any, check: boolean) => {
      ...
      if (check) {
        newValue = values.branch.filter((i) => i !== item.id);
      } else {
        newValue = values.branch.concat(item.id);
      }
      setFieldValue('branch', newValue);
    },
    [values.branch, setFieldValue],
  );

  const branchText = useMemo(() => {
    let result = branches?.filter((i) => {
      return values.branch?.some((item) => item === i.id);
    });
